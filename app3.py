...
<style>
:root{
  --bg:#EEF3F0; --bg-soft:#F4F8F6; --surface:#FFFFFF; --surface-2:#F4F8F6; --inset:#F5F9F7;
  --border:#E1E9E4; --border-soft:#EDF2EF;
  --text:#17241F; --muted:#5F6F68; --faint:#9AA6A0;
  --green:#0E9C66; --green-deep:#0A7E57; --green-soft:#E6F4EC;
  --accent:#0E9C66; --accent-soft:#E6F4EC;
  --danger:#C0392B; --warn:#B7791F;
  --radius:14px; --radius-sm:10px;
  --shadow:0 12px 34px -22px rgba(16,60,40,.28);
}
*{box-sizing:border-box;margin:0;padding:0}
html{scroll-behavior:smooth}
body{font-family:'Inter',system-ui,sans-serif;color:var(--text);min-height:100vh;
  -webkit-font-smoothing:antialiased;letter-spacing:.005em;background:
  radial-gradient(1100px 540px at 82% -12%, rgba(14,156,102,.08), transparent 60%),
  radial-gradient(820px 460px at -6% 0%, rgba(10,126,87,.06), transparent 55%),
  var(--bg)}

/* ---------- Header (logo + onglets + meta) ---------- */
.topbar{position:sticky;top:0;z-index:50;display:flex;align-items:center;gap:20px;flex-wrap:wrap;
  padding:13px 26px;background:rgba(255,255,255,.9);backdrop-filter:blur(14px);
  border-bottom:1px solid var(--border);box-shadow:0 1px 0 rgba(16,60,40,.02)}
.brand{display:flex;align-items:center}
.brand img{height:26px;display:block}
.tag{font-size:.8rem;color:var(--muted);font-weight:500;padding-left:18px;
  border-left:1px solid var(--border);letter-spacing:.01em}
.topmeta{margin-left:auto;display:flex;align-items:center;gap:8px;font-size:.74rem;color:var(--faint);font-weight:500}
.dot{width:7px;height:7px;border-radius:50%;background:var(--green);box-shadow:0 0 0 3px var(--green-soft)}

/* onglets dans le header */
.switch{position:relative;display:flex;gap:5px;padding:5px;background:var(--surface-2);
  border:1px solid var(--border);border-radius:14px}
.switch-slider{position:absolute;top:5px;left:5px;height:calc(100% - 10px);border-radius:10px;
  background:linear-gradient(135deg,var(--green-deep),var(--green));
  box-shadow:0 8px 18px -8px var(--green-deep),inset 0 1px 0 rgba(255,255,255,.25);
  transition:left .32s cubic-bezier(.5,.05,.2,1),width .32s cubic-bezier(.5,.05,.2,1)}
.tab{position:relative;z-index:1;display:flex;align-items:center;gap:10px;padding:8px 16px;border:none;
  background:none;cursor:pointer;border-radius:10px;color:var(--muted);font-family:inherit;text-align:left;
  transition:color .25s}
.tab.active{color:#fff}
.tab .tab-ic{display:grid;place-items:center;width:28px;height:28px;border-radius:8px;background:#fff;
  border:1px solid var(--border);transition:.25s;flex-shrink:0;color:var(--muted)}
.tab.active .tab-ic{background:rgba(255,255,255,.22);border-color:transparent;color:#fff}
.tab .tab-ic svg{width:15px;height:15px}
.tab b{display:block;font-family:'Space Grotesk',sans-serif;font-size:.88rem;font-weight:600;line-height:1.15}
.tab small{display:block;font-size:.66rem;font-weight:500;opacity:.85;letter-spacing:.02em}
...
.panel[data-tool="presta"]{--accent:#0E8C8A;--accent-soft:#E3F4F2}
...
.eyebrow span{font-family:'Space Grotesk',sans-serif;font-size:.74rem;font-weight:600;letter-spacing:.16em;
  text-transform:uppercase;color:var(--accent)}
.eyebrow .src{margin-left:auto;font-size:.72rem;color:var(--faint);font-weight:500;letter-spacing:.02em}

/* ---------- Form ---------- */
.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(215px,1fr));gap:16px}
.field label{display:block;font-size:.7rem;font-weight:600;color:var(--muted);text-transform:uppercase;
...
.ms-btn{width:100%;padding:11px 13px;background:var(--inset);border:1px solid var(--border);
  border-radius:var(--radius-sm);font-size:.9rem;cursor:pointer;display:flex;justify-content:space-between;
  align-items:center;gap:8px;user-select:none;color:var(--text);transition:border-color .2s}
...
.ms-pop{display:none;position:absolute;z-index:40;top:calc(100% + 6px);left:0;right:0;background:#fff;
  border:1px solid var(--border);border-radius:var(--radius-sm);box-shadow:0 18px 40px -16px rgba(16,60,40,.28);
  max-height:262px;overflow-y:auto;padding:6px}
.ms-opt{display:flex;align-items:center;gap:10px;padding:8px 9px;font-size:.85rem;border-radius:7px;
  cursor:pointer;color:var(--text)}
.ms-opt:hover{background:var(--accent-soft)}
.ms-opt input{appearance:none;width:17px;height:17px;border:1.5px solid var(--border);border-radius:5px;
...
.btn{padding:12px 22px;border-radius:var(--radius-sm);border:1px solid transparent;
  ...
  display:inline-flex;align-items:center;gap:9px;transition:transform .12s,box-shadow .2s,background .2s,opacity .2s}
.btn:active{transform:translateY(1px)}
.btn svg{width:16px;height:16px}
.btn-primary{background:linear-gradient(135deg,var(--green-deep),var(--green));color:#fff;
  box-shadow:0 10px 24px -12px var(--green-deep)}
.btn-primary:hover{box-shadow:0 14px 28px -10px var(--green-deep)}
...
/* ---------- Status + progress ---------- */
.status{display:none;margin-top:16px;padding:11px 15px;border-radius:var(--radius-sm);font-size:.86rem;
...
.status.done{background:var(--green-soft);color:var(--green-deep);border-color:#BFE6D0}
...
.bar{height:100%;width:0;border-radius:4px;background:linear-gradient(90deg,var(--green-deep),var(--green),var(--green-deep));
  background-size:200% 100%;transition:width .4s ease;animation:sheen 1.6s linear infinite}
@keyframes sheen{to{background-position:200% 0}}

/* ---------- Stats ---------- */
.stats{display:flex;gap:14px;flex-wrap:wrap;margin-bottom:18px}
.stat{flex:1;min-width:130px;background:var(--bg-soft);border:1px solid var(--border);
  border-radius:var(--radius-sm);padding:14px 16px}
.stat .n{font-family:'Space Grotesk',sans-serif;font-size:1.7rem;font-weight:700;line-height:1;color:var(--text)}
...
thead th{position:sticky;top:0;z-index:1;background:var(--bg-soft);color:var(--muted);text-align:left;
  font-weight:600;font-size:.72rem;letter-spacing:.06em;text-transform:uppercase;padding:12px 14px;
...
tbody tr:nth-child(even){background:rgba(14,156,102,.025)}
tbody tr:hover{background:var(--accent-soft)}
tbody tr:last-child td{border-bottom:none}
code.ref{font-family:ui-monospace,'SF Mono',Menlo,monospace;font-size:.78rem;color:var(--accent);font-weight:600}
...
@media (max-width:760px){
  .tag{display:none}
  .tab small{display:none} .tab{padding:8px 13px}
...
<div class="topbar">
  <a class="brand" href="#" onclick="return false"><img src="__LOGO_SRC__" alt="NeoProprio"/></a>
  <span class="tag">Extracteur d'opérations certifiées</span>
  <div class="switch" id="switch">
    <div class="switch-slider" id="slider"></div>
    <button class="tab active" data-tool="nf" onclick="switchTab('nf')">
      <span class="tab-ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 11l9-7 9 7v9a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1z"/><path d="M9 21v-6h6v6"/></svg></span>
      <span><b>NF Habitat</b><small>Cerqual · Qualitel</small></span>
    </button>
    <button class="tab" data-tool="presta" onclick="switchTab('presta')">
      <span class="tab-ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2l8 4v6c0 5-3.5 8-8 10-4.5-2-8-5-8-10V6z"/><path d="M9 12l2 2 4-4"/></svg></span>
      <span><b>Prestaterre BEE</b><small>Labels BEE</small></span>
    </button>
  </div>
  <div class="topmeta"><span class="dot"></span> Données en direct</div>
</div>
...
function positionSlider(){
  ...
  const tab=sw.querySelector('.tab.active');
  const sl=document.getElementById('slider');
  sl.style.width=tab.offsetWidth+'px';
  sl.style.left=(tab.offsetLeft-sw.clientLeft)+'px';
}
...
if(document.fonts&&document.fonts.ready){document.fonts.ready.then(positionSlider);}
